...
class AttackPrompt(object):
    ...
    def _update_ids(self):
        ...
        self.conv_template.append_message(self.conv_template.roles[0], f"{self.goal} {self.control} {self.defense}")
        ...
        if self.conv_template.name == 'llama-2':
            ...
            self.conv_template.update_last_message(f"{self.goal}{separator}{self.control}{separator}{self.defense}")
            toks = self.tokenizer(self.conv_template.get_prompt()).input_ids
            self._defense_slice = slice(self._control_slice.stop, len(toks))
    ...
    def generate(self, model, gen_config=None):
        ...
        return output_ids#output_ids[self._assistant_role_slice.stop:]
    
    def generate_str(self, model, gen_config=None):
        print(self.generate(model, gen_config))
    ...
    @defense_str.setter
    def defense_str(self, defense):
        self.defense = defense
        self._update_ids()
    
    @property
    def defense_toks(self):
        return self.input_ids[self._defense_slice]
    
    @property
    def get_defense_slice(self):
        return self._defense_slice
...
class PromptManager(object):
    ...
    @property
    def get_defense_slice(self):
        return self._prompts[0].get_defense_slice
...
class MultiPromptAttack(object):
    ...
    @property
    def get_defense_slice(self):
        return [prompts.get_defense_slice for prompts in self.prompts]
        ...
        for step, control in enumerate(controls):
            for (mode, goals, targets) in zip(*[('Train', 'Test'), (self.goals, self.test_goals), (self.targets, self.test_targets)]):
                if control != prev_control and len(goals) > 0:
                    ...
                    defense_slice_total = [p.get_defense_slice for p in attack.prompts[0]]
                    ...
                    for i in range(len(all_inputs) // batch_size + 1):
                        ...
                        defense_slice_l = defense_slice_total[i*batch_size:(i+1)*batch_size]
