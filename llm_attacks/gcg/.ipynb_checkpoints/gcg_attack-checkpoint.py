...
def token_gradients(model, input_ids, input_slice, target_slice, loss_slice, defense_slice):
    ...
    final_embeds = torch.cat(
        [
            full_embeds[:,:defense_slice.start,:], 
            defense_embeddings_torch.clone().detach().unsqueeze(0), 
            embeds[:,defense_slice.stop:,:]
        ], 
        dim=1)
    
    logits = model(inputs_embeds=final_embeds).logits
        ...
        with torch.no_grad():
            for j, cand in enumerate(control_cands):
                ...
                for i in progress:
                    ...
                    #print(self.workers[0].tokenizer.decode(ids[0][0]))
