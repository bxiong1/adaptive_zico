...
_MODELS = {
    "LLaMA-2-7B": ["/root/autodl-tmp/Llama-2-7b-chat-fp16", {"use_fast": False}, "llama-2", 64]
}
...
def main(_):
    ...
    for model in _MODELS:
        ...
        for goal, target, control in zip(goals, targets, controls):
            ...
            controls = [control]
            test_inputs.append(train_goals[0])
        ...
        test_outputs=[]
        count = 0
        with open('/root/autodl-tmp/llm-attacks/output_txt.txt', 'w') as f:
            for line in test_total_outputs:
                f.write(f"Batch_{count}\n{line[0]}\n")
                test_outputs.append(str(line[0]))
                count=count+1
        df = pd.DataFrame({'Inputs':test_inputs,'Outputs':test_outputs})
        df.to_csv('/root/autodl-tmp/llm-attacks/output_txt.csv', sep=',', encoding='utf-8')
